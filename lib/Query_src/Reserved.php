...
class Reserved {
    ...
    public static $reservedKey = true;
    public static $reservedValue = true;
     ...
     * @return mixed
     */
    protected function replaceReservedWords($key, $value = false) {
        if (!self::$reservedKey && !$value)
            return $key;

        if (!self::$reservedValue && $value)
            return $key;
        ...
        if (in_array($key, $new)) {
            ...
            return strpos($key, '`') ? $key : (string) '`' . $key . '`';
        }
